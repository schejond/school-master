...
//            findBestPlay(neighbourBoard,neighbourMoves,neighbourRemainingSoldiers,currentDepth + 1);
        ...
        if (currentDepth <= SOLDIERS_CNT/2) { // zkousel jsem take statickou hranici <= 5, ale ta neni dostatecne obecna
            # pragma omp task
                findBestPlay(neighbourBoard,neighbourMoves,neighbourRemainingSoldiers,currentDepth + 1);
        } else {
            findBestPlay(neighbourBoard, neighbourMoves, neighbourRemainingSoldiers, currentDepth + 1);
        ...
        // nevhodne, extremne zpomali beh programu pro tuto implementaci
//        # pragma omp taskwait
    }
}
...
//    printBoardState(initialBoardState->boardState);
    ...
    cout << FILE_NAME << "," << secs << "." << remainingMs << "," << CALL_CNT << "," << argv[2] << ",1,ompTask" << endl;
