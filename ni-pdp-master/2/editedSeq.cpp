...
public:
    ...
    BoardState(vector<char> boardState, const unsigned short &remainingSoldiers, const short &bishopPosition, const short &knightPosition)
...
void findBestPlay(BoardState &currentBoard, vector<BoardMove> &currentMoves,
    ...
    while (!movesToExplore.empty()) {
        ...
        findBestPlay(neighbourBoard,neighbourMoves,neighbourRemainingSoldiers,currentDepth + 1);
    }
}
...
BoardState* loadInstance(const string &fileName) {
    ...
    return new BoardState(boardState, SOLDIERS_CNT, bishopPosition, knightPosition);
}
...
//    printBoardState(initialBoardState.boardState);
    ...
    vector<bool> b(MAX_DEPTH, false);
    vector<BoardMove> moves(MAX_DEPTH, BoardMove(-1));
    findBestPlay(*initialBoardState, moves, SOLDIERS_CNT, 0);
