...
//                System.out.println("Reading file: " + file.getName());
    ...
    private Solution parseSolutionContentFromString(final String line) throws Exception {
        List<String> splitedString = Arrays.asList(line.split(" "));
        List<Integer> values = new ArrayList<>();
        splitedString.forEach((val) -> values.add(Integer.valueOf(val)));
        ...
        List<Boolean> solutions = new ArrayList<>();//(values.subList(3, values.size()));
        for (int i : values.subList(3, values.size())) {
            solutions.add(i == 1);
        }
